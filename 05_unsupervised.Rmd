...
* Exclude the columns `world_rank`, `institution`, `country`, `national_rank`, `year` from the data
* Pass the ranking variables to `kmeans`
* Try several `centers` values to explore whethere there are some variables for clustering

```{r}
dt_km %>% 
  ggplot(aes(quality_of_faculty, citations)) +
  geom_point()
...
row.names(sum_dt) <- sum_dt$country
...
sum_dt$.cluster <- factor(res_km$cluster, levels = 1:3)

sum_dt %>% 
  ggplot(aes(quality_of_education, alumni_employment, color = .cluster)) +
...
res_km <-
  dt %>%
  filter(country == "USA") %>% 
  select(quality_of_education, quality_of_faculty) %>%
  kmeans(centers = 3, nstart = 50)
...
dt_km <- augment(res_km, dt_usa)

dt_km %>% 
  ggplot(aes(quality_of_education,
             quality_of_faculty)) +
  geom_point()
...
dt_km %>%
  filter(quality_of_education < 50,
         quality_of_faculty < 50)
```


```{r}
row.names(dt_km) <- dt_km$institution
plot(as.phylo(hclust(dist(dt_km))), labels = dt_km$institution, type = "unrooted", no.margin = TRUE, cex = 0.6)
```

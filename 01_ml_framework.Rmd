...
The idea behind cross-validation is to allow the analyst check whether they're overfitting the data without predicting on the `r colorize("testing", "#D4FF2A")` data. How does it work? First, we **only** work with our `r colorize("training", "red")` data
...
The result of `predict_training` is the `r colorize("training", "red")` data from `age_inc` with one new column: the predicted values of the model. Let's visualize them:

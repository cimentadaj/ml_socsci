...
for (i in 1:(n-1)) {
  ...
  df_pred[[paste0("pred_", i)]] <- 0.01 * predict(res[[i]], new_data = pull_tflow_rawdata(res[[i]]))[[1]]
  pred_data <-
    cbind(
...
- The number of trees for a RF does not lead to overfitting because the average reduces the variance

- The number of trees in GB does affect overfitting because you are sequentially building up on the previous one and adding more trees can learn noise from very small residuals. That's why we use the learning stop.

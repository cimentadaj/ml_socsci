...
#### 9. Fit an untuned boosting algorithm to check whether the default values are enough to beat the other models {-#ex9}

From the example in the main text, we saw that boosting had the highest accuracy out of all models. However, can we improve it? This exercise is exploratory. Try running a `boost_tree` through a grid search of `trees`, `learn_rate`, `tree_depth` and `min_n`. To avoid running models for too long, set `levels = 3` in `plug_grid`.

* Define `boost_tree` with `tune()` for all the parameters defined above
* Replace the previous model with this model
* Replace the previous formula to exclude `scie_score` and `read_score` with `plug_formula`
* Plug a cross-validation with `vfold_cv` and a grid with `grid_regular`
* Fit the grid search

Is there a particular set of tuning parameters that maximizes accuracy? Note that our initial prediction with only 500 trees on the training set already achieved a very small error of less than one point in test scores. What you're looking for is to test whether the model overfits the data less such that the test error is improved.
...
tflow <-
  ...
  replace_formula(math_score ~ . - scie_score - read_score) %>%
  plug_resample(vfold_cv) %>%
  plug_grid(grid_regular, levels = 3)
